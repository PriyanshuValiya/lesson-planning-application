...
function ViewLessonPlanPage() {
  ...
  if (!lessonPlan) {
    return (
      <div className="container mx-auto py-6">
        <div className="text-center py-12">
          <p className="text-lg text-red-500">Lesson plan not found or you don&apos;t have permission to view it.</p>
        </div>
      </div>
    )
  }
  ...
  const getSectionNumber = () => {
    ...
    if (!isSubjectPracticalOnly(lessonPlan.subject)) {
      sections.push({ number: sectionNumber++, name: "UNIT DETAILS", type: "units" })
    }

    // Only add Practical Details for non-theory-only subjects
    if (!isSubjectTheoryOnly(lessonPlan.subject)) {
      sections.push({ number: sectionNumber++, name: "PRACTICAL DETAILS", type: "practicals" })
    }
        ...
        <CardContent className="overflow-hidden">
          ...
          {sections.map((section) => {
                        ...
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td className="border border-black p-2" colSpan={3}>
                              {unit.co_mapping
                                .map((coId) => {
                                  const outcome = lessonPlan.courseOutcomes.find((co) => co.id === coId)
                      ...
                      <table className="w-full border-collapse table-fixed">
                        ...
                        <tbody>
                          {lessonPlan.cies.map((cie: any, index: number) => (
                            <tr key={index}>
                              ...
                              <td className="border border-black p-2 break-words overflow-hidden text-ellipsis max-w-0">
                                {cie.co_mapping
                                  .map((coId: any) => {
                                    const outcome = lessonPlan.courseOutcomes.find((co: any) => co.id === coId)
                                    return outcome ? outcome.text : coId
                                  })
                                  .join(", ")}
                              </td>
                          ...
                          <tr className="font-bold">
                            ...
                            <td className="border border-black p-2 text-center break-words overflow-hidden text-ellipsis max-w-0">
                              {lessonPlan.cies.reduce((sum: number, cie: any) => sum + cie.marks, 0)}
                            </td>
                            <td className="border border-black p-2 text-center break-words overflow-hidden text-ellipsis max-w-0"></td>
                            <td className="border border-black p-2 text-center break-words overflow-hidden text-ellipsis max-w-0"></td>
                          </tr>
            ...
            if (section.type === "additional") {
              return (
                <div key="additional" className="mb-6">
                  <h2 className="text-lg font-bold mb-2">
                    {section.number}. {section.name}
                  </h2>
