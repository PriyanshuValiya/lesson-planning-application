  ...
  useEffect(() => {
    const loadCopyLP = async () => {
      ...
      try {
        ...
        const { data: allSubjectsWithCode, error: subjectsError } =
          await supabase
            .from("subjects")
        ...
        if (!allSubjectsWithCode || allSubjectsWithCode.length <= 1) {
          console.log(
            "No other subjects found with code:",
        ...
        const { data: facultyWithSubjects, error: facultyError } =
          await supabase
            .from("user_role")
        ...
        console.log(
          "Found faculty with same subject code:",
          facultyWithSubjects?.length
        );
    ...
    try {
      ...
      for (const subject of commonSubject) {
        if (subject.id !== lessonPlan.id) {
          console.log(
            `📋 Checking ${subject.users.name} - ${subject.subjects.name} (${subject.departments.name})`
          );
          ...
          if (formData && formData.length > 0) {
            ...
            if (form.form && trueCount > 0) {
              ...
              console.log(
                `    ⚠️ Form has ${trueCount} TRUE flags or no form data - skipping`
              );
            }
          } else {
      ...
      allForms.forEach((form, index) => {
        console.log(
          `${index + 1}. ${form.faculty.name} (${form.department.name}) - ${
            form.subject.name
          }: ${form.trueCount}/5 TRUE flags`
        );
      });
      ...
      console.log(
        "🔍 DEBUG: Source additionalInfo data:",
        formData.additionalInfo
      );
      console.log(
        "🔍 DEBUG: Current lessonPlan.additionalInfo:",
      ...
      setLessonPlan((prevLessonPlan) => {
        const updatedPlan = {
          ...
          lab_hours:
            formData.generalDetails?.lab_hours || prevLessonPlan.lab_hours,
          ...
          course_outcomes:
            formData.generalDetails?.courseOutcomes ||
            prevLessonPlan.course_outcomes,
          ...
          practicalPlanning:
            formData.practicalPlanning || prevLessonPlan.practicalPlanning,
          practicals:
          ...
          cieDetails:
            formData.ciePlanning?.cieDetails ||
            formData.cieDetails ||
            prevLessonPlan.cieDetails,
          cie_components:
            formData.ciePlanning?.cie_components ||
            formData.cie_components ||
            prevLessonPlan.cie_components,
          cies:
            formData.ciePlanning?.cies || formData.cies || prevLessonPlan.cies,
          ...
          additional_info: {
            ...
            classroom_conduct:
              formData.additionalInfo?.classroom_conduct ||
              prevLessonPlan.additional_info?.classroom_conduct,
            attendance_policy:
              formData.additionalInfo?.attendance_policy ||
              prevLessonPlan.additional_info?.attendance_policy,
            ...
            cie_guidelines:
              formData.additionalInfo?.cie_guidelines ||
              prevLessonPlan.additional_info?.cie_guidelines,
            self_study_guidelines:
              formData.additionalInfo?.self_study_guidelines ||
              prevLessonPlan.additional_info?.self_study_guidelines,
            topics_beyond_syllabus:
              formData.additionalInfo?.topics_beyond_syllabus ||
              prevLessonPlan.additional_info?.topics_beyond_syllabus,
            reference_materials:
              formData.additionalInfo?.reference_materials ||
              prevLessonPlan.additional_info?.reference_materials,
            academic_integrity:
              formData.additionalInfo?.academic_integrity ||
              prevLessonPlan.additional_info?.academic_integrity,
            communication_channels:
              formData.additionalInfo?.communication_channels ||
              prevLessonPlan.additional_info?.communication_channels,
            ...
            fast_learner_planning:
              formData.additionalInfo?.fast_learner_planning ||
              prevLessonPlan.additional_info?.fast_learner_planning,
            ...
            slow_learner_planning:
              formData.additionalInfo?.slow_learner_planning ||
              prevLessonPlan.additional_info?.slow_learner_planning,

            // Copy events if available
            events:
              formData.additionalInfo?.events ||
              prevLessonPlan.additional_info?.events ||
              [],

            // 🚫 KEEP existing PDF file URLs - DO NOT copy them
            fast_learner_file_url:
              prevLessonPlan.additional_info?.fast_learner_file_url,
            medium_learner_file_url:
              prevLessonPlan.additional_info?.medium_learner_file_url,
            slow_learner_file_url:
              prevLessonPlan.additional_info?.slow_learner_file_url,
          },
        ...
        console.log(
          "🔍 DEBUG: Updated lessonPlan.additional_info:",
          updatedPlan.additional_info
        );
        console.log("🔍 DEBUG: Specific fields copied:");
        console.log(
          "  - classroom_conduct:",
          updatedPlan.additional_info?.classroom_conduct
        );
        console.log(
          "  - attendance_policy:",
          updatedPlan.additional_info?.attendance_policy
        );
        console.log(
          "  - lesson_planning_guidelines:",
          updatedPlan.additional_info?.lesson_planning_guidelines
        );
        ...
        >
          ...
          >
            ...
            {showPracticalPlanning && (
              <TabsTrigger value="practical-planning">
                Practical Planning
              </TabsTrigger>
            )}
