...
export default function ViewActualCie({
  ...
  const formatUnitNumber = (unitNum: string) => {
    ...
    console.log(
      "[v0] Invalid unit number detected:",
      unitNum,
      "parsed as:",
      num
    );
  ...
  const getPracticalDisplay = (practicalIds: string[], formsData: any) => {
    ...
    return practicalIds
      .map((practicalId) => {
        const practical = formsData.form.practicals?.find(
          (p: any) => p.id === practicalId
        );
        if (practical) {
          // Extract practical number from ID or use a counter
          const practicalIndex =
            formsData.form.practicals.findIndex(
              (p: any) => p.id === practicalId
            ) + 1;
  ...
  const formatActualUnits = (actualUnits: string, formsData: any) => {
    if (!actualUnits) return "-";

    console.log("[v0] Processing actual_units:", actualUnits);

    const unitIds = actualUnits.split(",").map((id) => id.trim());
    return unitIds
      .map((unitId) => {
        // First try to find it as a unit ID in the form data
        const unitIndex = formsData.form?.units?.findIndex(
          (u: any) => u.id === unitId
        );
        ...
        // If not found as ID, try to parse as a direct number
        const directNum = Number.parseInt(unitId.trim());
        if (directNum > 0 && directNum <= 10) {
          return `${directNum}`;
        }

        console.log("[v0] Could not process unit:", unitId);
        return "-";
      })
      .filter((unit) => unit !== "-") // Filter out invalid units
      .join(", ");
  };
            ...
            <Table className="border-1 border-gray-300">
              ...
              <TableBody>
                <TableRow className="text-center" key={`row-${cie.id}`}>
                  ...
                  <TableCell className="border-1 border-gray-200">
                    {isThisPractical
                      ? // Display practicals for practical CIEs
                        Array.isArray(cie.practicals_covered) &&
                      ...
                      ? cie.units_covered
                          .map((unitId) => {
                            const unitIndex =
                              formsData.form.unitPlanning.units.findIndex(
                                (u) => u.id === unitId
                              ) + 1;
                  ...
                  <TableCell className="border border-gray-200 text-center">
                    <div>
                      {[
                        ...(Array.isArray(cie.co_mapping)
                          ? cie.co_mapping.map((_, i) => `CO${i + 1}`)
                          : []),
                        ...(Array.isArray(cie.pso_mapping)
                          ? cie.pso_mapping.map((_, i) => `PSO${i + 1}`)
                          : []),
                      ].join(", ")}
                    </div>
                  </TableCell>
                  <TableCell className="border border-gray-200 text-center">
                    <div className="whitespace-normal w-50 mx-auto text-center">
                      {getDisplayPedagogy(
                        ...
                        cie.custom_pedagogy || cie.other_pedagogy
                      )}
                    </div>
                  </TableCell>
                  <TableCell className="border border-gray-200 text-center">
                    <div className="flex flex-wrap justify-center gap-1">
                      {Array.isArray(cie.blooms_taxonomy)
                ...
                <TableRow className="text-center " key={`row2-${actual.id}`}>
                  ...
                  <TableCell className="border-1 border-gray-200">
                    {isThisPractical
                      ? // Display actual practicals for practical CIEs
                        ...
                        : "-"
                      : // Use improved formatActualUnits function for better unit display
                      actual.actual_units
                      ? formatActualUnits(actual.actual_units, formsData)
                      : "-"}
                  </TableCell>
                  <TableCell className="border-1 border-gray-200">
                    {actual.actual_date
                      ? format(parseISO(actual.actual_date), "dd-MM-yyyy")
                      : "-"}
                  </TableCell>
                  <TableCell className=" border-1 border-gray-200">
                  ...
                  <TableCell className="border border-gray-200 text-center">
                    {(actual.co?.split(",").map((_, i) => `CO${i + 1}`) ?? [])
                      .concat(
                        actual.pso?.split(",").map((_, i) => `PSO${i + 1}`) ??
                          []
                      )
                      .join(", ")}
                  </TableCell>
              ...
              <TableFooter>
                <TableRow>
                  ...
                  >
                    <p
                      className={`whitespace-normal mx-auto text-sm leading-tight w-20`}
                    >
                    ...
                    <div className="mt-1">
                      {actual.moderation_start_date ? (
                        format(
                          parseISO(actual.moderation_start_date),
                          "dd-MM-yyyy"
                        )
                      ) : (
                      ...
                      {actual.moderation_end_date ? (
                        format(
                          parseISO(actual.moderation_end_date),
                          "dd-MM-yyyy"
                        )
                      ) : (
                        <p className="text-xs">Not Reviewed</p>
                  ...
                  >
                    <p
                      className={`flex flex-col whitespace-normal mx-auto text-sm leading-tight w-20`}
                    >
                      <p>View</p>
                      <button
                  ...
                  >
                    ...
                    >
                      ...
                      <button
                        ...
                        onClick={() =>
                          handleViewDocument(
                            actual.moderation_report_document,
                            "Moderation Report"
                          )
                        }
                      >
                        {actual.moderation_report_document ? (
                          <p className="mt-1">Moderation Report</p>
                        ) : (
                          <p className="text-xs mt-1">Not Uploaded</p>
                        )}
                      </button>
