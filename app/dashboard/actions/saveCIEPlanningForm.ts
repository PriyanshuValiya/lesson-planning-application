        ...
        if (form && form.cies && Array.isArray(form.cies)) {
          for (let i = 0; i < form.cies.length; i++) {
            ...
            if (existingCIE.date) {
              ...
              // Skip date conflict check if same subject code
              ...
              if (existingSubjectInfo && existingSubjectInfo.code === currentSubject.code) {
                continue // Skip same subject codes
              }
              ...
              if (cieDate === existingCIEDate) {
                ...
                errors.push(
                  `CIE ${cieIndex + 1}: There is another ${existingCIE.type || "CIE"} taken by ${facultyName} on the same date for ${subjectName}`
                )
                break
              }
            }
