...
export async function saveCIEPlanningForm(formData: CIEPlanningFormData) {
  try {
    ...
    // Validate date gaps using proper date parsing
    const sortedDates = dates
      .map((date) => ({ original: date, parsed: parseDDMMYYYY(date) }))
      .filter((item) => item.parsed !== null)
      .sort((a, b) => a.parsed!.getTime() - b.parsed!.getTime())

    for (let i = 1; i < sortedDates.length; i++) {
      const prevDate = sortedDates[i - 1].parsed!
      const currDate = sortedDates[i].parsed!
      const daysDiff = Math.abs((currDate.getTime() - prevDate.getTime()) / (1000 * 60 * 60 * 24))
