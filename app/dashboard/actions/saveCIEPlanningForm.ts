...
//     contentData: {
//       hasUnits:
//         form.units && Array.isArray(form.units) && form.units.length > 0,
//       hasPracticals:
//         form.practicals &&
//         Array.isArray(form.practicals) &&
//         form.practicals.length > 0,
//     },
//   });
...
//     console.log(
//       "🔍 BACKEND: Subject type determined as theory_practical (from flags)"
//     );
//     return "theory_practical";
//   } else if (
//     subjectData?.is_practical === true &&
//     subjectData?.is_theory === false
//   ) {
//     console.log(
//       "🔍 BACKEND: Subject type determined as practical (from flags)"
//     );
//     return "practical";
//   } else if (
//     subjectData?.is_theory === true &&
//     subjectData?.is_practical === false
//   ) {
...
//     console.log(
//       "🔍 BACKEND: Subject type determined as theory_practical (from content)"
//     );
//     return "theory_practical";
//   } else if (hasPracticals && !hasUnits) {
//     console.log(
//       "🔍 BACKEND: Subject type determined as practical (from content)"
//     );
//     return "practical";
//   } else {
...
//     .from("subjects")
//     .select("semester, department_id, code, name")
//     .eq("id", subjectId)
//     .single()

//     if (subjectError || !currentSubject) {
//       console.error(
//         "Error fetching current subject for conflict check:",
//         subjectError
//       );
//       return errors;
//     }
...
//       .in(
//         "id",
//         allForms.map((record: any) => record.faculty_id)
//       );

//     if (facultyError) {
//       console.error("Error fetching faculty2 details:", facultyError);
//     }
...
//             if (existingCIE.date) {
//               const existingCIEDate = convertToStandardDateFormat(
//                 existingCIE.date
//               );

//               // Skip if it's the same CIE being edited
//               if (
//                 formRecord.subject_id === subjectId &&
//                 formRecord.faculty_id === facultyId &&
//                 i === cieIndex
//               ) {
//                 continue;
//               }
...
//                 // Find faculty name
//                 const facultyInfo = facultiesData?.find(
//                   (f: any) => f.id === formRecord.faculty_id
//                 );
...
//                 // Find subject name
//                 const subjectInfo = sameSubjects.find(
//                   (s: any) => s.id === formRecord.subject_id
//                 );
//                 const subjectName = subjectInfo
//                   ? `${subjectInfo.name} ${
//                       subjectInfo.code ? `(${subjectInfo.code})` : ""
//                     }`
//                   : "another subject";

//                 errors.push(
//                   `CIE ${cieIndex + 1}: There is another ${
//                     existingCIE.type || "CIE"
//                   } taken by ${facultyName} on the same date for ${subjectName}`
//                 );
//                 break;
//               }
//             }
...
//   console.log(
//     "🔍 VALIDATION: Running critical validation only for performance"
//   );
//   console.log("🔍 VALIDATION: Subject type:", subjectType);

//   // NEW: Check for CIE date conflicts
//   const conflictErrors = await checkCIEDateConflicts(
//     supabase,
//     cies,
//     subjectId,
//     facultyId
...
//         : "";
//     errors.push(
//       `At least one CIE from each required type must be present. Missing: ${missingTypes.join(
//         ", "
//       )}${semesterNote}`
//     );
//   }
...
//         errors.push(
//           `CIE ${index + 1} date cannot exceed the Course Term End Date`
//         );
//       }
//     });
//   }
...
//       errors.push(
//         `CIE ${index + 1}: At least one Bloom's taxonomy level is required`
//       );
//     }
//   });
...
//       "Mid-term/Internal Exam",
//     ];
//     const theoryCIEs = cies.filter((cie: any) =>
...
//   } else {
//     console.log(
//       "🔍 VALIDATION: Skipping theory CIE duration validation for practical-only subject"
//     );
//   }

//   console.log(
//     "🔍 VALIDATION: Critical validation completed with errors:",
//     errors
//   );
//   return errors;
// };
...
//     const [
//       { data: existingForm, error: formError },
...
//         success: false,
//         error:
//           "No lesson plan found. Please complete the lesson plan first before adding CIE planning.",
//       };
//     }
...
//           } else {
//             termEndDate = parseDDMMYYYYToDate(
//               subjectData.metadata.term_end_date
//             );
//           }
//         }
//       }
...
//     if (
//       form.generalDetails?.courseOutcomes &&
//       Array.isArray(form.generalDetails.courseOutcomes)
//     ) {
...
//       return {
//         success: false,
//         error: `Failed to save CIE planning: ${
...
// UPDATED: Function to check for CIE date conflicts with subject code validation
const checkCIEDateConflicts = async (
  supabase: any,
  cies: CIEData[],
  subjectId: string,
  ...
  try {
    // Get current subject's semester, department, and code
    const { data: currentSubject, error: subjectError } = await supabase
      .from("subjects")
      .select("semester, department_id, code, name")
      .eq("id", subjectId)
    ...
    console.log("🔍 CONFLICT CHECK: Current subject:", {
      id: subjectId,
        ...
        if (form && form.cies && Array.isArray(form.cies)) {
          for (let i = 0; i < form.cies.length; i++) {
            ...
            if (existingCIE.date) {
              ...
              // UPDATED: Skip date conflict check if same subject code
              const existingSubjectInfo = sameSubjects.find((s: any) => s.id === formRecord.subject_id)

              if (existingSubjectInfo && existingSubjectInfo.code === currentSubject.code) {
                console.log("🔍 CONFLICT CHECK: Skipping conflict check for same subject code:", {
                  currentSubjectCode: currentSubject.code,
                  existingSubjectCode: existingSubjectInfo.code,
                  currentSubjectName: currentSubject.name,
                  existingSubjectName: existingSubjectInfo.name,
                })
                continue
              }
...
const validateCriticalRequirements = async (
  ...
  // NEW: Check for CIE date conflicts (now with subject code validation)
...
export async function saveCIEPlanningForm(formData: CIEPlanningFormData) {
  ...
  try {
    ...
    // OPTIMIZED: Run only critical validations for performance (including date conflict check with subject code validation)
    const criticalErrors = await validateCriticalRequirements(
      supabase,
      formData.cies,
      subjectType,
