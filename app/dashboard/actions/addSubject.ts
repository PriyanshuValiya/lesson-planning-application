...
export async function addSubject(formData: FormData) {
  try {
    ...
    const lectureHours = Number.parseInt(formData.get("lectureHours") as string)
...
export async function deleteSubject(id: string) {
  try {
    ...
    const { data: facultyWithOnlyThisSubject, error: facultyError } = await (await supabase)
      .from("user_role")
      .select("id, user_id")
      .eq("subject_id", id)
    ...
    const { error: deleteError } = await (await supabase).from("subjects").delete().eq("id", id)
