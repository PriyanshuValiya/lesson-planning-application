...
import { Plus, Trash2, AlertTriangle, Info, Users, Save } from "lucide-react";
...
export default function CIEPlanningForm({
  ...
  const [facultySharingError, setFacultySharingError] = useState<string | null>(
    null
  );
  ...
  const checkFacultySharing = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const cleanedResult = {
          ...result,
          primaryFaculty: result.primaryFaculty
            ? getFacultyName(result.primaryFaculty)
            : null,
  ...
  useEffect(() => {
    const loadDraft = async () => {
      ...
      const hasExistingData =
        ...
        currentCIEs.some(
          (cie: any) =>
            cie.type && cie.type !== "" && cie.date && cie.date !== ""
        );
            ...
            const validCIEs = data.cies.map((cie: any, index: number) => ({
              ...
              skill_mapping:
                Array.isArray(cie.skill_mapping) && cie.skill_mapping.length > 0
                  ...
                  : [{ skill: "", details: "" }],
              assigned_faculty_id:
                cie.assigned_faculty_id || userData?.id || null, // ADDED: Load saved assignment or default to current user
            }));
  ...
  const validateAllCIEs = () => {
    ...
    // NEW VALIDATION: Check if number of theory CIEs is less than 3
    const theoryCIEs = currentCIEs.filter(
      (cie: any) => cie.type === "Theory CIE"
    );
    if (theoryCIEs.length < 3) {
      errors.push("Theory CIEs should be planned at least 3 in a semester");
    }
    ...
    const practicalCIEs = currentCIEs.filter((cie: any) =>
      ["Practical CIE", "Internal Practical"].includes(cie.type)
    );
    ...
    const onlylectureCIEs = currentCIEs.filter(
      (cie: any) => cie.type === "Theory CIE"
    );
    const lecturePedagogies = onlylectureCIEs
    ...
    if (onlylectureCIEs.length > 0 && usedTraditionalInLecture.length === 0) {
      errors.push(
        "At least one traditional pedagogy method must be used in Theory CIEs"
      );
    }
    ...
    if (usedTraditionalInLecture.length !== uniqueTraditionalInLecture.size) {
      errors.push(
        "Each traditional pedagogy method must be used only once across Theory CIEs"
      );
    }
      ...
      if (!hasPrereqCIE) {
        ...
        if (firstSemRelevantCIEs.length > 0) {
          ...
          if (totalCOs > 0 && allCOMappings.size < totalCOs) {
            errors.push(
              "All COs must be covered across Theory CIEs + Mid-term/Internal Exams"
            );
          }
        }
      } else {
  ...
  const handleSave = async () => {
    ...
    if (facultySharingData?.isSharing && !currentCIE.assigned_faculty_id) {
      allErrors.push(
        `CIE ${activeCIE + 1}: Please assign this CIE to a faculty member`
      );
    }
    ...
    try {
      ...
      if (facultySharingData?.isSharing) {
        const unassignedCIEs = (lessonPlan.cies || []).filter(
          (cie: any) => !cie.assigned_faculty_id
        );
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-600">
                  ...
                  assign each CIE assessment to the appropriate faculty.
                </p>
              </div>
            </div>
            <Badge
          ...
          <div className="mt-3 grid grid-cols-2 gap-4">
            ...
            <div className="bg-white rounded p-3 border border-blue-200">
              ...
              <p className="font-semibold text-blue-800">
                ...
                facultySharingData.allFaculty.length > 1
                  ? facultySharingData.allFaculty
                      .filter(
                        (f: any) =>
                          getFacultyName(f) !==
                          facultySharingData.primaryFaculty
                      )
          ...
          <div className="flex items-start">
            ...
            <div className="text-blue-800">
              ...
              <p className="text-sm">
                ...
                across Theory CIEs and Mid-term/Internal Exam only.
              </p>
            </div>
          </div>
        </div>
              ...
              title={
                facultySharingData?.isSharing && cie.assigned_faculty_id
                  ? `Assigned to: ${getFacultyName(
                      facultySharingData.allFaculty?.find(
                        (f: any) => f.id === cie.assigned_faculty_id
                      )
                    )}`
                  : undefined
              }
        ...
        {facultySharingData?.isSharing && (
          <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-lg px-4 py-3">
            ...
            <Select
              ...
              onValueChange={(value) =>
                handleCIEChange(activeCIE, "assigned_faculty_id", value)
              }
            >
        ...
        <div className="flex justify-between items-center pt-6 border-t">
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {saving ? "Submitting..." : "Submit CIE Planning"}
            </Button>
          </div>
        </div>
      </div>
