...
export default function PracticalPlanningForm({ lessonPlan, setLessonPlan }: PracticalPlanningFormProps) {
  ...
  useEffect(() => {
    const loadFacultySharing = async () => {
      ...
      try {
        const result = await checkFacultySharing(lessonPlan.subject.id)
        console.log("Practical Faculty sharing result:", result) // Add this for debugging
  ...
  const onSubmit = async (data: PracticalPlanningFormValues) => {
    ...
    if (isSharing) {
      const unassignedPracticals = data.practicals.filter((practical) => !practical.assigned_faculty_id)
      if (unassignedPracticals.length > 0) {
        toast.error("Please assign faculty to all practicals before saving.")
        return
      }
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
      ...
      {practicalFields[activePractical] && (
        <>
          {/* Faculty Sharing Status for Practicals */}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex flex-col gap-3">
                ...
                {/* Faculty Assignment Dropdown - Always show for debugging */}
