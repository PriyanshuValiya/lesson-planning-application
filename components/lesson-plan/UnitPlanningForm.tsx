...
//                   {getShortFacultyName(watch(`units.${index}.assigned_faculty_id`))}
//                 </Badge>
//               )}
//             </Button>
//           ))}
...
export default function UnitPlanningForm({ lessonPlan, setLessonPlan }: UnitPlanningFormProps) {
  ...
  useEffect(() => {
    const loadFacultySharing = async () => {
      ...
      try {
        const result = await checkFacultySharing(lessonPlan.subject.id)
        console.log("Faculty sharing result:", result) // Add this for debugging
  ...
  const getFacultyAssignmentStats = () => {
    const assignments = unitFields.reduce(
      (acc, unit, index) => {
        const facultyId = watch(`units.${index}.assigned_faculty_id`)
        if (facultyId) {
          acc[facultyId] = (acc[facultyId] || 0) + 1
        }
        return acc
      },
    ...
    return Object.entries(assignments).map(([facultyId, count]) => ({
      faculty: getFacultyName(facultyId),
      count,
    }))
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
      ...
      {/* Faculty Sharing Status - Always visible for shared subjects */}
      <div className="bg-gradient-to-r from-blue-50 to-green-50 border-2 border-blue-200 rounded-lg p-4 mb-6">
        ...
        <div className="mt-3 grid grid-cols-2 gap-4">
          <div className="bg-white rounded p-3 border border-blue-200">
            ...
            <p className="font-semibold text-blue-800">Dr. Parth Goel</p>
          </div>
          <div className="bg-white rounded p-3 border border-blue-200">
            <span className="text-sm font-medium text-gray-700">Secondary Faculty:</span>
            <p className="font-semibold text-blue-800">Dr. Secondary Faculty</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            {unitFields.map((unit, index) => {
              ...
              return (
                <div key={unit.id} className="flex items-center justify-between bg-white rounded p-2 border">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex flex-col gap-3">
              ...
              {/* Faculty Assignment Dropdown - Always show for debugging, then make conditional */}
              <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-lg px-4 py-3">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="faculty1">Dr. A</SelectItem>
                    <SelectItem value="faculty2">Dr. B</SelectItem>
                    <SelectItem value={userData?.id || "current"}>{userData?.name || "Current User"}</SelectItem>
                  </SelectContent>
                </Select>
